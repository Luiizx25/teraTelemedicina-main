...
@section('subcontent')
    <div class="grid grid-cols-12 gap-2">
        <div class="intro-y col-span-12 flex flex-wrap sm:flex-no-wrap items-center mt-2">
            <h2 class="text-2xl font-medium truncate">{{__('Providers Specialties')}}</h2>
            <div class="ml-auto mt-2">
                <a href="javascript:;" id="modal-provider-specialty-add-open-btn" data-toggle="modal" data-target="#modal-provider-specialty-add" class="button text-white bg-theme-1 shadow-md ml-auto">
                    {{__('New specialty')}}
                </a>
            </div>
            <!-- MODAL -->
            <div class="modal" id="modal-provider-specialty-add">
                <div class="modal__content modal__content--xl">
                    <form id="form_provider_add" action="{{route('toManager.providerSpecialty.store')}}" method="post">
                        @csrf
                        <div class="intro-y col-span-12 sm:col-span-12 lg:col-span-4 box shadow-md border border-solid border-gray-300">
                            <div class="flex items-center px-5 py-2 border-b border-gray-200 dark:border-dark-5">
                                <div class="mr-auto">
                                    <h2 class="font-medium text-base mr-auto">{{__('New Provider Specialty')}}</h2>
                                </div>
            ...
            <table id="table" class="table table-report -mt-2 display">
                <thead>
                    <tr>
                        ...
                        <th class="hover:bg-gray-200 whitespace-no-wrap">{{__('Specialty')}}</th>
                ...
                <tbody>
                    @php
                        $count = $providerSpecialties->count();
                    @endphp
                    @forelse ($providerSpecialties as $specialtyKey => $specialtyItem)
                        <tr class="intro-x shadow hover:shadow-lg">
                            <td class="">
                                <div class="flex items-center justify-center {{ $specialtyItem->ref_to_view ? 'text-theme-9' : 'text-theme-6' }}">
                                    <i data-feather="{{ $specialtyItem->ref_to_view?'activity':'slash'}}" class="w-4 h-4 mr-1"></i>
                                </div>
                            </td>
                            <td class="">
                                {{$specialtyItem->ref_description}}
                            </td>
                            <td>
                                {{$specialtyItem->ref_placeholder}}
                            </td>
                            <td class="table-report__action w-20">
                                @if ($specialtyItem->customer_sys_id??false)
                                <a href="javascript:;" id="btn-provider-open-modal" data-toggle="modal" data-target="#modal-provider-edit-{{$specialtyItem->id}}" class="">
                                    <div class="flex items-center justify-center {{ $specialtyItem->to_view ? 'text-theme-9' : 'text-theme-6' }}">
                                ...
                                <!-- MODAL -->
                                @if ($specialtyItem->customer_sys_id??false)
                                <div class="modal" id="modal-provider-edit-{{$specialtyItem->id}}">
                                    <div class="modal__content modal__content--xl">
                                        <form id="form_provider_add" action="{{route('toManager.providerSpecialty.update',['providerSpecialty'=>$specialtyItem->id])}}" method="post">
                                            @csrf
                                            @method('PUT')
                                            <input type="hidden" name="ref_slug" value="{{$specialtyItem->ref_slug}}">

                                            <div class="intro-y col-span-12 sm:col-span-12 lg:col-span-4 box shadow-md border border-solid border-gray-300">
                                                <div class="flex items-center px-5 py-2 border-b border-gray-200 dark:border-dark-5">
                                                    <div class="mr-auto">
                                                        <h2 class="font-medium text-base mr-auto">{{__('Edit provider')}}</h2>
                                                    </div>
                                                    @if (session('status_error'))
                                                    <div class="mt-2 px-2 py-1 bg-theme-6 items-center text-indigo-100 leading-none lg:rounded-full flex lg:inline-flex" role="alert">
                                                        <span class="flex rounded-full bg-indigo-100 text-theme-6 uppercase px-2 py-1 text-xs font-bold mr-3">Ops!</span>
                                                ...
                                                <div class="p-5">
                                                    ...
                                                    <div class="grid grid-cols-12 gap-2 mt-4">
                                                        <div class="col-span-12 sm:col-span-4 lg:col-span-4">
                                                            <div class="mb-2">{{__('Description')}}</div>
                                                            <input type="text" id="ref_description-{{$specialtyKey}}" name="ref_description" value="{{old('ref_description',$specialtyItem->ref_description)}}" class="input w-full border flex-1" aria-required="" required>
                                                        </div>
                                                        <div class="col-span-12 sm:col-span-8 lg:col-span-8">
                                                            <div class="mb-2">{{__('Placeholder')}}</div>
                                                            <input type="text" id="ref_placeholder-{{$specialtyKey}}" name="ref_placeholder" value="{{old('ref_placeholder',$specialtyItem->ref_placeholder)}}" class="input w-full border flex-1">
                                                        </div>
                                                    </div>

                                                    <div class="grid grid-cols-12 gap-2 mt-4">
                                                        <div class="intro-y col-span-12 sm:col-span-12 lg:col-span-6 text-left">
                                                            <a href="javascript:;" data-toggle="modal" data-target="#delete-modal-{{$specialtyKey}}" class="button inline-block bg-theme-6 text-white w-full sm:w-1/2 lg:w-auto mt-2">{{__('Delete provider')}}</a>
                                                        </div>
                                        ...
                                        <!-- -->
                                        <div class="modal" id="delete-modal-{{$specialtyKey}}">
                                            <div class="modal__content">
                                                <form action="{{route('toManager.providerSpecialty.destroy',['providerSpecialty'=>$specialtyItem->id])}}" method="post">
                                                    @csrf
                                                    @method('DELETE')
...
@section('script')
    <script>
        ...
        {
            // OPEN MODAL
            cash('#modal-provider-specialty-add').modal('show')
        }
