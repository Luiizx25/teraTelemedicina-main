...
@section('subcontent')
    <div class="grid grid-cols-12 gap-2">
        <div class="intro-y col-span-12 flex flex-wrap sm:flex-no-wrap items-center mt-2">
            <h2 class="text-2xl font-medium my-2 truncate">{{__('Variations Services')}}</h2>
            <div class="ml-auto mt-2">
                <a href="{{route('toManager.serviceVariation.create')}}" class="button text-white bg-theme-1 shadow-md ml-auto">
                    {{__('New Variation')}}
                </a>
            </div>
        </div>
        ...
        <div class="box intro-y col-span-12 overflow-auto lg:overflow-visible p-4">
            <table id="table_services" class="table table-report -mt-2 display">
                <thead>
                    <tr>
                        <th class="hover:bg-gray-200 whitespace-no-wrap">{{__('Service')}}</th>
                        <th class="hover:bg-gray-200 whitespace-no-wrap">{{__('Variation')}}</th>
                        <th class="">{{__('')}}</th>
                    </tr>
                </thead>
                <tbody>
                    ...
                    @foreach ($services as $serviceKey => $serviceItem)
                        <!--  -->
                        @foreach ($serviceItem->Variations as $variation)
                            <tr class="intro-x shadow hover:shadow-lg">
                                ...
                                <td class="text-center">
                                    <a href="{{route('toManager.serviceVariation.edit',['serviceVariation'=> $variation->slug ])}}"class="flex hover:text-orange-600">
