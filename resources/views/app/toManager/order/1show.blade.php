                ...
                <div class="flex flex-1 items-center justify-center lg:justify-start">
                    <div class="m-2 ">
                        <div class="w-20 h-20 image-fit">
                            @empty($order->customer->cus_logo)
                            <img class="rounded" src="{{asset('/app/images/default_profile.png')}}">
                            @else
                            <img class="rounded" src="{{asset('storage/' . $order->customer->cus_logo)}}">
                            @endempty
                        </div>
                    </div>
                    <div class="mr-auto">
                        <div class="font-medium text-base">{{$order->customer->cus_name??'---'}}
                            @if ($order->customer->cus_name_company)
                                <p class="text-gray-600 text-xs">{{$order->customer->cus_name_company}}</p>
                            @endif
                        </div>
                        <p class="text-xs">{{ strtoupper($order->customer->cus_doc_type) }} {{$order->customer->cus_doc_num}}</p>
                        <p class="inline-block text-xs text-white px-1 bg-theme-1 rounded">{{__('Customer')}} {{$order->customer->id}}</p>
                    </div>
                </div>
            </div>
