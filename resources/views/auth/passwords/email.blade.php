...
@section('content')
    ...
    <div class="h-screen xl:h-auto flex py-5 xl:py-0 my-10 xl:my-0">
        <div class="my-auto mx-auto xl:ml-20 bg-white xl:bg-transparent px-5 sm:px-8 py-8 xl:p-0 rounded-md shadow-md xl:shadow-none w-full sm:w-3/4 lg:w-2/4 xl:w-auto">
            ...
            <form id="send-form" method="POST" action="{{ route('password.email') }}">
                <div class="intro-x mt-4">
                    @csrf
                    <input id="email" name="email" type="email" class="intro-x login__input input input--lg border border-gray-300 block @error('email') border-theme-6 @enderror mt-4" placeholder="Email" value="{{ old('email') }}" required>
                    @error('email')
                        <div id="error-email" class="email__input-error w-5/6 text-theme-6 mt-2"><strong>{{ __($message) }}</strong></div>
                    @enderror
                </div>
                <div class="intro-x mt-5 xl:mt-8 text-center xl:text-left">
                    <button id="btn-send" class="button button--lg w-50 xl:w-100 text-white bg-theme-1 xl:mr-3 align-top">
